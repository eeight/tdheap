...
MemBlock *NewMemBlock(Addr start_addr, SizeT size) {
  MemBlock *block = VG_(malloc)("heap1.newmemblock.1", sizeof(*block));
  
  block->start_addr = start_addr;
  block->size = size;
  block->used_from = VG_(HT_construct)("heap1.newmemblock.2");
  block->reads_count = 0;
  ...
  block->map = VG_(HT_construct)("heap1.newmemblock.3");
